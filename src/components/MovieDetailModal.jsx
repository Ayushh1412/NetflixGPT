    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black bg-opacity-80 p-4">
        ...
        >
          ...
          {/* Poster Section */}
          <div className="w-full md:w-1/3 h-64 md:h-auto relative hidden md:block">
             <img
          ...
          {/* Details Section */}
          <div className="w-full md:w-2/3 p-6 text-white overflow-y-auto custom-scrollbar">
            {loading ? (
              ...
              details && (
                <>
                  ...
                  <div className="flex items-center gap-4 text-sm text-gray-400 mb-4">
                    ...
                    <span className="border px-1 rounded border-gray-600">{details.Rated}</span>
                    <span>{details.Runtime}</span>
                    <span className="text-yellow-400 font-bold">★ {details.imdbRating}</span>
                  </div>
                  
                  <div className="mb-4">
                    <p className="text-gray-300 italic mb-4">{details.Genre}</p>
                    <p className="text-lg leading-relaxed">{details.Plot}</p>
                  </div>
                  ...
                  <div className="border-t border-gray-700 pt-4 mt-6">
                    <h3 className="text-xl font-semibold mb-3 flex items-center gap-2">
                       <span className="text-red-500">GPT</span> Related Picks
                    </h3>
                    
                    {loadingRelated ? (
                         <div className="text-gray-500 italic">Thinking of suggestions...</div>
                    ) : (
                        <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                            {relatedMovies.length > 0 ? relatedMovies.map(movie => (
                                <div 
                                    ...
                                    onClick={() => dispatch(openModal(movie))} // Click to view that movie!
                                >
                                    <img 
                                    ...
                                    />
                                    <p className="text-xs mt-1 truncate text-gray-400">{movie.Title}</p>
                                </div>
                            )) : (
